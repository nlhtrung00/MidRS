...
from app.forms import LoginFrom, RegisterForm, UploadDataForm, UploadTemplateForm, TypeForm, AlgorithmForm
...
def file_page():
    form = UploadDataForm()
    files = Files.query.filter_by(user_id=current_user.id, is_template=False)
    ...
    # path = os.path.join(parent_path, f'uploads\{filename}')
    ...
    if form.validate_on_submit():
        ...
        if check_binary(data[:,2]):
            if form.select.data == '5' or form.select.data == '8' or form.select.data == '1' or form.select.data == '2':
                ...
                if form.select.data == '5':
                    rs = CollaborativeFiltering(Y_data = Y_train, Y_test=Y_test, amount = amount, similarity_based=JACCARD, based=USER)
                    rs.fit()
                    ...
                    db.session.commit()
                    precision, recall = rs.evaluate(Y_test)
                    f1 = 0
                    ...
                    rs = CollaborativeFiltering(Y_data = Y_train, Y_test=Y_test, amount = amount, similarity_based=JACCARD, based=ITEM)
                    rs.fit()
                    with open(storage_path_result, 'w', newline="") as f:
                        write = csv.writer(f, delimiter=" ")
                        write.writerows(rs.recommendation_result())
            ...
            if form.select.data != '5' and form.select.data != '8':
                ...
                if form.select.data == '2':
                    ...
                    rs = CollaborativeFiltering(Y_data = Y_train, Y_test= Y_test, amount = amount, similarity_based=COSINE, based=USER)
                    rs.fit()
                    with open(storage_path_result, 'w', newline="") as f:
                        write = csv.writer(f, delimiter=" ")
                        write.writerows(rs.recommendation_result())
                    ...
                    rs = CollaborativeFiltering(Y_data = Y_train, Y_test=Y_test, amount = amount, similarity_based=PEARSON, based=USER)
                    rs.fit()
                    with open(storage_path_result, 'w', newline="") as f:
                        write = csv.writer(f, delimiter=" ")
                        write.writerows(rs.recommendation_result())
                    ...
                    rs = CollaborativeFiltering(Y_data = Y_train, Y_test=Y_test, amount = amount, similarity_based=COSINE, based=ITEM)
                    rs.fit()
                    with open(storage_path_result, 'w', newline="") as f:
                        write = csv.writer(f, delimiter=" ")
                        write.writerows(rs.recommendation_result())
                    ...
                    rs = CollaborativeFiltering(Y_data = Y_train, Y_test=Y_test, amount = amount, similarity_based=PEARSON, based=ITEM)
                    rs.fit()
                    with open(storage_path_result, 'w', newline="") as f:
                        write = csv.writer(f, delimiter=" ")
                        write.writerows(rs.recommendation_result())
                    ...
                    rs_cf_cosine_user = CollaborativeFiltering(Y_data = Y_train, Y_test= Y_test, amount = amount, similarity_based=COSINE, based=USER)
                    rs_cf_cosine_user.fit()

                    rs_cf_pearson_user = CollaborativeFiltering(Y_data = Y_train, Y_test= Y_test, amount = amount, similarity_based=PEARSON, based=USER)
                    rs_cf_pearson_user.fit()

                    rs_cf_cosine_item = CollaborativeFiltering(Y_data = Y_train, Y_test= Y_test, amount = amount, similarity_based=COSINE, based=ITEM)
                    rs_cf_cosine_item.fit()

                    rs_cf_pearson_item = CollaborativeFiltering(Y_data = Y_train, Y_test= Y_test, amount = amount, similarity_based=PEARSON, based=ITEM)
        ...
        #     rs = CollaborativeFiltering(Y_train, k=2, similarity_based=CONSINE, based=USER)
        #     rs.fit()
        #     with open(path, 'w', newline="") as f:
        #         write = csv.writer(f,delimiter=" ")
        #         write.writerows(rs.recommendation_result())

        #     file = Files(name=form.file.data.filename, link=path,
        #                  user_id=current_user.id, is_template=0)
        #     db.session.add(file)
        #     db.session.commit()
        #     flash('Tải tập tin lên thành công!', category='success')
        #     return redirect(url_for('file_page'))
        # elif form.select.data == '3':
        #     rs = CollaborativeFiltering(Y_train, k=2, similarity_based=PEARSON, based=USER)
        #     rs.fit()
        #     with open(path, 'w', newline="") as f:
        #         write = csv.writer(f,delimiter=" ")
        #         write.writerows(rs.recommendation_result())
        ...
        # elif form.select.data == '4':
        #     if check_binary(data[:,2]):
        #         rs = CollaborativeFiltering(Y_train, k=2, similarity_based=JACCARD, based=USER)
        #         rs.fit()
        #         with open(path, 'w', newline="") as f:
        #             write = csv.writer(f,delimiter=" ")
        #             write.writerows(rs.recommendation_result())
        ...
        #         return redirect(url_for('file_page'))
        #     else:
        #         flash('Jaccard được tính trên dữ liệu nhị phân!', category='warning')
        # elif form.select.data == '5':
        #     rs = CollaborativeFiltering(Y_train, k=2, similarity_based=CONSINE, based=ITEM)
        #     rs.fit()
        #     with open(path, 'w', newline="") as f:
        #         write = csv.writer(f,delimiter=" ")
        #         write.writerows(rs.recommendation_result())
        ...
        #     return redirect(url_for('file_page'))
        # elif form.select.data == '6':
        #     rs = CollaborativeFiltering(Y_train, k=2, similarity_based=PEARSON, based=ITEM)
        #     rs.fit()
        #     with open(path, 'w', newline="") as f:
        #         write = csv.writer(f,delimiter=" ")
        #         write.writerows(rs.recommendation_result())
        ...
        # elif form.select.data == '7':
        #     if check_binary(data[:,2]):
        #         rs = CollaborativeFiltering(Y_train, k=2, similarity_based=JACCARD, based=ITEM)
        #         rs.fit()
        #         with open(path, 'w', newline="") as f:
        #             write = csv.writer(f,delimiter=" ")
        #             write.writerows(rs.recommendation_result())
        ...
        #         flash('Jaccard được tính trên dữ liệu nhị phân!', category='warning')
        # elif form.select.data == '8':
        #     rs = MatrixFactorizattion(Y_train, latent=10, regularization=.1,
        #                   eta=0.70, max_iterations=100, based=USER)
        #     rs.fit()
        #     with open(path, 'w', newline="") as f:
        #             write = csv.writer(f,delimiter=" ")
        #             write.writerows(rs.recommendation_result())
        ...
        # else:
        #     if check_binary(data[:,2]):
        #         rs_jaccard_user = CollaborativeFiltering(Y_train, k=2, similarity_based=JACCARD, based=USER)
        #         rs_jaccard_user.fit()
        #         rs_jaccard_item = CollaborativeFiltering(Y_train, k=2, similarity_based=JACCARD, based=ITEM)
        #         rs_jaccard_item.fit()
        #         if rs_jaccard_user.evaluate(Y_test)[0] > rs_jaccard_item.evaluate(Y_test)[0]:
        #             with open(path, 'w', newline="") as f:
        #                 write = csv.writer(f,delimiter=" ")
        #                 write.writerows(rs_jaccard_user.recommendation_result())
        ...
        #             with open(path, 'w', newline="") as f:
        #                 write = csv.writer(f,delimiter=" ")
        #                 write.writerows(rs_jaccard_item.recommendation_result())
        ...
        #             return redirect(url_for('file_page'))
        #     else:
        #         rs_popular = Popular(Y_train)
        #         rs_popular.fit()
        #         rs_cosine_user = CollaborativeFiltering(Y_train, k=2, similarity_based=CONSINE, based=USER)
        #         rs_cosine_user.fit()
        #         rs_cosine_item = CollaborativeFiltering(Y_train, k=2, similarity_based=CONSINE, based=ITEM)
        #         rs_cosine_item.fit()
        #         rs_pearson_user = CollaborativeFiltering(Y_train, k=2, similarity_based=PEARSON, based=USER)
        #         rs_pearson_user.fit()
        #         rs_pearson_item = CollaborativeFiltering(Y_train, k=2, similarity_based=PEARSON, based=ITEM)
        #         rs_pearson_item.fit()
        #         rs_mf_user = MatrixFactorizattion(Y_train, latent=10, regularization=.1,
        #                   eta=0.70, max_iterations=100, based=USER)
        #         rs_mf_user.fit()
        #         # rs_mf_item = MatrixFactorizattion(Y_train, latent=10, regularization=.1,
        #         #             eta=0.70, max_iterations=100, based=ITEM)
        #         # rs_mf_item.fit()
        #         eva_popular = rs_popular.evaluate(Y_test)[0]
        #         eva_cosine_user = rs_cosine_user.evaluate(Y_test)[0]
        #         eva_cosine_item = rs_cosine_item.evaluate(Y_test)[0]
        #         eva_pearson_user = rs_pearson_user.evaluate(Y_test)[0]
        #         eva_pearson_item = rs_pearson_item.evaluate(Y_test)[0]
        #         eva_mf_user = rs_mf_user.evaluate(Y_test)[0]
        #         # eva_mf_item = rs_mf_item.evaluate(Y_test)[0]
        #         array_eva = [eva_popular, eva_cosine_user, eva_cosine_item, eva_pearson_user, eva_pearson_item, eva_mf_user]
        #         index_max = array_eva.index(min(array_eva))
        #         if index_max == 0:
        #             with open(path, 'w', newline="") as f:
        #                 write = csv.writer(f,delimiter=" ")
        ...
        #             return redirect(url_for('file_page'))
        #         elif index_max == 1:
        #             print(rs_cosine_user.recommendation_result())
        #             with open(path, 'w', newline="") as f:
        #                 write = csv.writer(f,delimiter=" ")
        #                 write.writerows(rs_popular.recommendation_result())
        ...
        #             with open(path, 'w', newline="") as f:
        #                 write = csv.writer(f,delimiter=" ")
        #                 write.writerows(rs_cosine_item.recommendation_result())
        ...
        #             with open(path, 'w', newline="") as f:
        #                 write = csv.writer(f,delimiter=" ")
        #                 write.writerows(rs_pearson_user.recommendation_result())
        ...
        #             with open(path, 'w', newline="") as f:
        #                 write = csv.writer(f,delimiter=" ")
        #                 write.writerows(rs_pearson_item.recommendation_result())
        ...
        #             with open(path, 'w', newline="") as f:
        #                 write = csv.writer(f,delimiter=" ")
        #                 write.writerows(rs_mf_user.recommendation_result())
        ...
        # ------------------------------------------------------
            ...
            #     with open(path, 'w', newline="") as f:
            #         write = csv.writer(f,delimiter=" ")
            #         write.writerows(rs_mf_item.recommendation_result())
        ...
        # link = 'uploads\\' + str(uuid.uuid4()) + filename
        # file = Files(name=filename, link=link,
        #              user_id=current_user.id, is_template=0)
        # db.session.add(file)
        # db.session.commit()
        # form.file.data.save(link)
        # flash('Tải tập tin lên thành công!', category='success')
        # return redirect(url_for('file_page'))
...
def result_page(id):
    ...
    #         if max < len(row): max = len(row)
    #         data.append(list(row[1:]))
    return render_template("result_page.html", results=results, algorithms=algorithms)
...
@login_required
def download_sample():
    file = Files.query.filter_by(is_template=1).first()
    parent_path = pathlib.Path(__file__).parent.parent.resolve()
...
def admin_page():
    check_is_admin()
    count_template = Files.query.filter_by(is_template=1).count()
    count_upload = Files.query.filter_by(is_template=0).count()
    data = {'template': count_template, 'upload': count_upload}
...
def admin_file_page():
    ...
    if form.validate_on_submit():
        filename = secure_filename(form.file.data.filename)
        storage_path = 'uploads\\' + str(uuid.uuid4()) + filename
